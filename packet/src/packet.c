...
void pack(Packet *pk, uint8_t *buf) {
...
#endif
    memcpy(buf, pk->data, pk->length - sizeof(pk->header)
#ifdef USE_SEQNUM
                              - sizeof(pk->seqnum)
#endif
#ifdef USE_LENGTH
    ...
    buf += pk->length - sizeof(pk->header)
#ifdef USE_SEQNUM
           - sizeof(pk->seqnum)
#endif
#ifdef USE_LENGTH
...
void unpack(uint8_t *buf, Packet *pk, pointer_t *map) {
    ...
    buf += sizeof(pk->command);
    pk->data = (uint8_t *)map[pk->command];
#endif
    memcpy(pk->data, buf, pk->length - sizeof(pk->header)
#ifdef USE_SEQNUM
                              - sizeof(pk->seqnum)
#endif
#ifdef USE_LENGTH
